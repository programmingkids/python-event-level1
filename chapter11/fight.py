...
class Fight:
    def __init__(self, width, height):
        ...
        label_player_c = tkinter.Label(top_frame, text="レベル", justify="left")
        label_player_c.place(x=230, y=300)
        label_player_c.config(font=labelfont)
        ...
        # プレイヤのレベルを表示するラベル
        self.label_player_level = tkinter.Label(top_frame, justify="left")
        self.label_player_level.place(x=350, y=300)
        self.label_player_level.config(font=labelfont)
    ...
    def start(self, player, monster, fight_end):
        ...
        self.label_player_level["text"] = self.player.level
    ...
    def fight(self):
        ...
        if self.monster.hp <= 0:
            ...
            self.add_message(self.player.name + "の経験値が「" + str(self.player.exp) + "」になりました")
            self.add_message("===========")
            if self.is_level_up():
                self.player.level += 1
                ...
                self.player.hp += 10
                self.label_player_level["text"] = self.player.level
                self.add_message(self.player.name + "がレベルアップしました")
                self.add_message(self.player.name + "の攻撃力が「" + str(self.player.attack) + "」になりました")
                self.add_message(self.player.name + "のHPが「" + str(self.player.hp) + "」になりました")
                self.add_message("===========")
    ...
    def is_level_up(self):
        ...
        for i in range(self.player.level):
            i = i + 1
            next_level_exp += i * 10
        if self.player.exp >= next_level_exp:
            return True
        else:
            return False
